          ...
          ` : `
            ...
            <div class="flex flex-wrap gap-3">
              ${(await Promise.all(recent.map(async (item) => {
                ...
                return `
                  <!-- 更小的卡片尺寸 - 调整了响应式宽度比例 -->
                  <div class="flex-shrink-0 w-full sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/6">
                    <div class="bg-white rounded-lg overflow-hidden shadow hover:shadow-md transition-all duration-300 h-full flex flex-col">
                      ...
                      `}
                      <!-- 文字部分添加背景和内边距 -->
                      <div class="p-2 bg-gray-50 flex-grow flex flex-col">
                        <div class="font-medium mb-1 text-xs truncate">${item.imdb_id}</div>
                        <a href="${item.acfun_url}" target="_blank" 
                           class="text-blue-600 hover:underline text-xs mb-1 inline-block">
